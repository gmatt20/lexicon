...
from app.services import conversation_exists, verify_token, query_user
...
def post_message(
    message_data: MessageCreate, session: SessionDep, user_data=Depends(verify_token)
) -> Message:
...
def get_convo_messages(
    ...
    if not conversation_exists(conversation_id, user.id, session):
        raise HTTPException(
            status_code=404, detail="Conversation not found with this ID"
        )

    messages = session.exec(
        select(Message).where(
            Message.user_id == user.id, Message.conversation_id == conversation_id
        )
    ).all()
